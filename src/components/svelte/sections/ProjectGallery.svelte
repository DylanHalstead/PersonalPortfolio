<!-- 
  ...
  A Svelte an opinionated project gallery with tag sorting, pagination, and a modal.
  
  @prop {CollectionEntry<'projects'>[]} projects - The projects to display
  @prop {CollectionEntry<'tags'>[]} tags - The tags that can be used to filter the projects
  
  @example
  ```astro
  <ProjectGallery client:only="svelte" projects={projects} tags={tags} />
  ```
-->
<script lang="ts">
...
<div>
  <h2 class="text-6xl/16 xl:text-7xl/20 2xl:text-8xl/26 font-medium tracking-tightest text-center mb-8">
    My Projects
  </h2>
  <div class="mb-8">
      ...
      >
        <div class="bg-background-900 border-2 border-text-50/25 rounded-xl transition-transform duration-200 hover:scale-105 cursor-pointer">
          <img 
            ...
            loading="lazy"
          />
          <div class="py-2 px-4 flex justify-between items-center text-text-100 border-t-2 border-text-50/25">
            <div class="flex flex-col items-start text-left min-w-0 flex-1">
              <h3 class="text-lg sm:text-xl md:text-2xl font-bold truncate w-full">{project.data.title}</h3>
              <p class="opacity-50 text-sm sm:text-base">{project.data.date.toLocaleDateString('en-US', { year: 'numeric', month: 'short'})}</p>
            </div>
            <div class="flex gap-1.5 sm:gap-2.5 items-center flex-shrink-0">
              {#each project.data.tags.sort((a, b) => {
                ...
                <span class="text-lg sm:text-xl size-7 sm:size-9 flex items-center justify-center bg-background-800 rounded-md p-1 sm:p-1.5 {selectedTags.includes(tag.id) && 'text-primary-400'}">
                  {#if i === 3 && project.data.tags.length > 4}
                    <span>+{project.data.tags.length - 3}</span>
                  {:else}
                    {@html cachedSvgs[tag.data.svgSrc] || cachedSvgs['/tags/default.svg']}
                  {/if}
                </span>
...
>
  <div class="relative">
    ...
    <div class="absolute bottom-4 left-4 right-4 text-white space-y-2">
      <div>
        <p class="text-base sm:text-lg opacity-50">{selectedProject?.data.date.toLocaleDateString('en-US', { year: 'numeric', month: 'long'})}</p>
        <h2 class="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-bold break-words leading-tight">{selectedProject?.data.title}</h2>
      </div>
